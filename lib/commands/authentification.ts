...
export class Authentification {
  ...
  private async copyAuthTemplates() {
    await fs.writeFile(
      ...
      await fs.readFile(path.join(__dirname, "../templates/auth/auth-middleware.ejs"), "utf-8")
    );
    
    await fs.writeFile(
      path.join(this.projectPath, "src/models/user.model.ts"),
      await fs.readFile(path.join(__dirname, "../templates/auth/user-model.ejs"), "utf-8")
    );
    
    await fs.writeFile(
      path.join(this.projectPath, "src/services/auth.service.ts"),
      await fs.readFile(path.join(__dirname, "../templates/auth/auth-service.ejs"), "utf-8")
    );
    
    await fs.writeFile(
      path.join(this.projectPath, "src/controllers/auth.controller.ts"),
      await fs.readFile(path.join(__dirname, "../templates/auth/auth-controller.ejs"), "utf-8")
    );
    
    await fs.writeFile(
      path.join(this.projectPath, "src/routes/auth.routes.ts"),
      await fs.readFile(path.join(__dirname, "../templates/auth/auth-routes.ejs"), "utf-8")
    );
  }
