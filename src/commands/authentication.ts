	...
	private async generateAuthFiles(model: ProjectModel): Promise<void> {
		const templates = [
			{
				...
				source: "common/auth/user-model.ejs",
			},
			{
				target: "src/models/role.model.ts",
				source: "common/auth/role-model.ejs",
			},
			{
				target: "src/services/auth.service.ts",
				source: "common/auth/auth-service.ejs",
			},
		];
		...
		if (this.framework === "Express") {
			templates.push({
				...
				source: "express/auth/middleware.ejs",
			});
				...
				templates.push(
					{
						...
						source: "express/auth/controller.ejs",
					},
					{
						target: "src/routes/auth.routes.ts",
						source: "express/auth/routes.ejs",
					},
				);
			}
			...
			templates.push({
				target: "src/middleware/auth.middleware.ts",
				source: "fastify/auth/middleware.ejs",
			});
			...
			if (this.isRest) {
				templates.push(
					{
						target: "src/controllers/auth.controller.ts",
						source: "fastify/auth/controller.ejs",
					},
					{
						target: "src/routes/auth.routes.ts",
						source: "fastify/auth/routes.ejs",
					},
				);
			}
		}
